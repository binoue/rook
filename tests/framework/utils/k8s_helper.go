...
// DebugGetLogsFromNamespace show logs for all containers in all pods in the namespace
func (k8sh *K8sHelper) DebugGetLogsFromNamespace(namespace, testName, platformName string) string {
	logger.Infof("Gathering logs for all pods in namespace %s", namespace)
	...
	k8sh.debugGetPodsLogs(pods, namespace, testName, platformName)
	return ""
}

func (k8sh *K8sHelper) debugGetPodsLogs(pods *v1.PodList, namespace, testName, platformName string) {
	for _, p := range pods.Items {
		k8sh.debugGetPodLogs(p, platformName, namespace, testName, false)
		if strings.Contains(p.Name, "operator") {
			// get the previous logs for the operator
			k8sh.debugGetPodLogs(p, platformName, namespace, testName, true)
		}
	}
}

func (k8sh *K8sHelper) debugGetPodLogs(pod v1.Pod, platformName, namespace, testName string, previousLog bool) {
	for _, container := range pod.Spec.InitContainers {
		l, err := k8sh.Kubectl("-n", pod.Namespace, "logs", pod.Name, "-c", container.Name)
		if err != nil {
			logger.Errorf("Cannot get logs for pod %s and container %s. %v", pod.Name, container.Name, err)
			return
		}
		fmt.Printf("init container %v in %v in %v log: %v\n", container.Name, pod.Name, pod.Namespace, l)
	}
	for _, container := range pod.Spec.Containers {
		l, err := k8sh.Kubectl("-n", pod.Namespace, "logs", pod.Name, "-c", container.Name)
		if err != nil {
