...
func runBlockCSITestLite(helper *clients.TestClient, k8sh *utils.K8sHelper, s suite.Suite, clusterNamespaces []string, systemNamespace string, version cephv1.CephVersionSpec) {
	...
	for _, cn := range clusterNamespaces {
		...
		defer blockTestDataCleanUp(helper, k8sh, s, cn, poolName, storageClassName, blockName, podName, true)
		setupBlockLite(helper, k8sh, s, cn, systemNamespace, poolName, storageClassName, blockName, podName, version)
	}
}
